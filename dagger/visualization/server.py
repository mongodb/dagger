...
def graph_obj_to_d3(g_obj):
    ...
    # maintain local list of nodes newly discovered (compared to collections.OrderedDict nodes) in this subgraph
    new_nodes = []
    ...
    for edge_type in g_obj.edges:
        ...
        for from_node in g_obj.edges[edge_type]:
            for to_node in g_obj.edges[edge_type][from_node]:
                link = {}
                link['source'] = flask.session['nodes'][from_node]['id']#g_node_copies[from_node].d3_index
                link['target'] = flask.session['nodes'][to_node]['id']#g_node_copies[to_node].d3_index
                link['type'] = map_edge_type_to_str(edge_type)
                link['linkId'] = str(flask.session['nodes'][from_node]['id']) + "," + str(flask.session['nodes'][to_node]['id'])
                link['weight'] = 1
...
def query(myquery):
    ...
    raise ValueError("Not a valid query. Query should be format: function_name node_name")
...
if __name__ == "__main__":
    ...
    dep_graph = graph.Graph("library_dependency_graph.json")
